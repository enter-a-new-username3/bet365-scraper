...
class Bet365AndroidSession(Session):
    ...
    def go_homepage(self):
        ...
        assert homepage_response.status_code == 200, f"Blocked by Cloudflare, bad IP or headers should be updated" if homepage_response.status_code == 403 else f"Unknown error while going to homepage: {homepage_response.status_code}"
        ...
        assert configuration_response.status_code == 200, f"Blocked while getting configuration, probably bad IP" if configuration_response.status_code == 500 else f"Unknown error while fetching configuration: {configuration_response.status_code}"
    ...
    def get_x_net_header(self, url: str, cookie_header: str, post_data: bytes) -> str:
        ...
        assert response.status_code == 200, "An error occured while generating token: " + response.text
